...
const outputFilePath = path.join(projectRoot, 'src/generated/knownTrackables.ts'); // Output file for generated arrays
const outputDir = path.dirname(outputFilePath); // Directory for output file

console.log('Generating trackables from types.ts...');

try {
    // Read the contents of the types.ts file (source of truth for resource/flag names)
    const typesContent = fs.readFileSync(typesFilePath, 'utf-8');

    /**
     * Extracts all string literal union members for a property in a given interface.
     * - Finds the interface block, then locates the property, and extracts all quoted strings.
     * - Excludes the generic 'string' type from the result.
     * @param content The full file content as a string.
     * @param interfaceName The name of the interface to search within.
     * @param propertyName The property whose union literals to extract.
     * @returns An array of unique, sorted string literals found for the property.
     */
    ...
    // Extract all known tracked resource names from TrackedResource['name']
    const resourceNames = extractUnionLiterals(typesContent, 'TrackedResource', 'name');
    // Extract all known acquired item flag names from AcquiredItemFlag['itemName']
    ...
    // Generate the output TypeScript file content with timestamp and robust comments
